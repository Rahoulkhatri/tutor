...
"[project]/app/api/student/offers/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const teacherIds = [
            ...new Set(offers.map((o)=>o.userId).filter(Boolean))
        ];
        ...
        const myConnections = await db.collection(__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$mongodb$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["CONNECTIONS_COLLECTION"]).find({
            studentId
        }).toArray();
        const connectionByTeacher = Object.fromEntries(myConnections.map((c)=>[
                c.teacherId,
                c.status
            ]));
        const courses = offers.map((o)=>{
            ...
            return {
                ...
                teacherInitials: teacher?.name ? teacher.name.trim().split(/\s+/).map((n)=>n[0]).join("").slice(0, 2).toUpperCase() : "T",
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d00c56d9._.js.map